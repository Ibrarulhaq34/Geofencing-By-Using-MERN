...
function AddGeoMap() {
    ...
    const saveMap = async()=>{
          ...
          const mapResponse = await axios.post("http://localhost:2000/api/getMapInfo", { name: name });
      console.log("mapResponse", mapResponse);

      // Set the mapInfo to the state
      setMapInfo(mapResponse);
      await axios.post("http://localhost:2000/api/addMap",{parentId:mapInfo.data._id,coordinates:new_path,color:color})
      .then(response=>{
        if(response){
